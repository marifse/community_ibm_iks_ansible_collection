...
from plugins.modules.ibm_container_storage_attachment_create import run_module

class TestIbmContainerStorageAttachmentCreate(unittest.TestCase):
    @patch("plugins.module_utils.auth.Authenticator.get_iam_token")
    @patch.object(Storage, "create_storage_attachment")
    def test_run_module_successful(self, mock_create_storage_attachment, mock_get_iam_token):
        mock_get_iam_token.return_value = "mocked_iam_token"
        mock_create_storage_attachment.return_value = (False, True, "mocked_storage_info")
        ...
        mock_create_storage_attachment.assert_called_once_with(args["vpcVolumeAttachmentConfig"])
