...
from plugins.modules.ibm_container_public_service_endpoint_enable import run_module

class TestIbmContainerPublicServiceEndpointEnable(unittest.TestCase):
    @patch("plugins.module_utils.auth.Authenticator.get_iam_token")
    @patch.object(Beta, "enablePublicServiceEndpoint")
    def test_run_module_successful(self, mock_enable_public_service_endpoint, mock_get_iam_token):
        mock_get_iam_token.return_value = "mocked_iam_token"
        mock_enable_public_service_endpoint.return_value = (False, True)
        ...
        mock_enable_public_service_endpoint.assert_called_once_with(args["config"])
